...
for (k in 1:n) {
  dat[k, 1] <- k
  dat[k, 2] <- choose(n, k) * (1/6)^k * (5/6)^(n-k)
  dat[k, 3] <- 0.5*(1+k)
}
```
...
colnames(d.payoff) <- c("Iterations", "Payoff", "CumExp")

for (i in 1:iterations) {
  ## Roll 6 dice and store the outcome
  outcome <- sample(1:6, 6, replace=TRUE)
  
  ## Compute the number of dice landing with 6
  n.six <- sum(outcome==6)
  ...
  ## Compute the cumulative expected payoff in the third column
...
```{r}
ggplot(d.payoff, aes(x=Iterations, y=CumExp)) + 
  geom_line(color=6) + 
  geom_point(color=6) + 
  ...
  labs(title="Monte Carlo simulation", x="Iterations", y="Cumulative expected payoff") + 
  theme_minimal() + 
...
* $\theta_\text{cur}$: the current position of theta
* $\theta_\text{pro}$: the proposed move, which is given by random draws from a normal distribution $\mathcal{N}(\theta_\text{cur},0.2^2)$
...
#### 2(a) _Metropolis algorithm implementation with R_
...
chainGenerator <- function(start,          # starting point
...
#### 2(b) _Visualization of three MCMC chains overlaid_
...
ggplot(chains.r, aes(x=iteration, y=theta, col=chainType)) + 
  ...
  ggtitle("Overlaying trace plots of three chains") + 
  theme_minimal()
```
...
ggplot(chains.r, aes(x=theta, col=chainType, fill=chainType)) +
  ...
  labs(title="Overlaying density plots of three chains") +
  theme_minimal()
```

By overlaying trace plots of three chains, we can easily see that the three chains converge to the same posterior distribution. To see it more clear, we overlay density plots of three chains and the densities overlap quite a bit.

#### 2(c) _Visualization of three MCMC chains concatenated_
...
ggplot(data=d.plot, mapping=aes(x=x, y=y)) +
  ...
  geom_area(aes(x=ifelse(x>hdi.r[1] & x<hdi.r[2], x, 0), y=y), 
...
#### 2(d) _Metropolis algorithm implementation with JAGS_
...
```{r}
knitr::kable(summary(results), caption="Summary results")
```
...
ggplot(data=chains.jags) + 
  geom_line(aes(x=index, y=theta, color=chainType)) + 
...
ggplot(data=d.plot, mapping=aes(x=x, y=y)) +
  geom_line(color="darkblue") +
  geom_area(aes(x=ifelse(x>hdi.jags[1] & x<hdi.jags[2], x, 0), y=y), 
            fill="lightblue", alpha=0.5) + 
...
ggplot(d.plot, aes(x=theta, color=method, fill=method)) + 
  ...
  labs(title="Density plots overlayed from R and JAGS")
```
...
ggplot(subset(df, subset=Group=="Smart Drug", select="Score"), aes(x=Score)) + 
  geom_histogram(bins=30, fill="#69b3a2", color="#e9ecef", alpha=0.9) + 
  ggtitle("Histogram of the IQ Score") +
  theme_minimal()
```

#### 3 (a) _Expected IQ score greater than 100_
...
knitr::kable(summary(results), caption="Summary of the results")
```
...
trace.mu <- ggplot(data=d.chains) + 
  geom_line(aes(x=index, y=mu, color=chainType)) + 
...
trace.sigma <- ggplot(data=d.chains) + 
  geom_line(aes(x=index, y=sigma, color=chainType)) + 
...
$$
\begin{aligned}
\text{Null hypothesis: }\quad & \mu\leq 100 \\
\text{Alternative hypothesis: }\quad & \mu>100
\end{aligned}
$$
...
knitr::kable(d.table, caption="Effect Size Metric")
```

The _cohen's d_ is around 0.3. According to the effect size metric in the table above, we can conclude the effect size is between small and medium.
...
## Calculate P[d<100] from empirical CDF
...
# Load the libraries
library(runjags)   # to communicate with JAGS
library(rjags)     # to communicate with JAGS

library(ggplot2)   # plotting library
library(GGally)    # plotting library
```

#### 4(a) _Visualization of the raw data_

```{r}
# Load the data
load("dataFit.RData")

# put the data into a data frame (to plot with ggplot)
df <- data.frame(x = x, y = y)
ggpairs(df) + ggtitle("Pairs plot") + theme_minimal()
```

#### 4(b) _Fitting different models_
...
knitr::kable(summary(results0), caption="Summary results for the null model")

## Summary results for linear model
knitr::kable(summary(results1), caption="Summary results for the linear model")

## Summary results for quadratic model
knitr::kable(summary(results2), caption="Summary results for the quadratic model")
```

#### 4(c) _Visualization of real data and model predictions_
...
#### _4(d) Deviance information certerion_
...
The lower DIC indicates a better fit to the data. The table shows that the quadratic model yields the smallest DIC, thus the best fit out of three models, which is also consistent with what the plot shows.