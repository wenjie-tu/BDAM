...
n <- nrow(dat)
for (k in 0:n) {
  dat[k+1, 1] <- k # the number dice showing 6 on the top face
...
We see from the summary results that 0.5 lies in the equi-tailed 95% interval $(0.493841, 0.8597397)$ and that psrf (convergence diagnostic) is very close to 1 (which indicates the current distribution converges to the target distribution).
...
* `ecdf()` function is used to compute the empirical cumulative distribution function and 100 is passed to the empirical CDF to calculate the posterior probability of $\mu>100$
* Alternatively, `mean()` function can serve the same purpose since it returns the proportion of samples satisfying the condition
...
Unsurprisingly, we obtain the same posterior probability for $d>0$ as we have for $\mu>100$ since these two in essence are the same. Therefore, we can conclude that the posterior probability of the effect size larger than 0 is 99.1%.
