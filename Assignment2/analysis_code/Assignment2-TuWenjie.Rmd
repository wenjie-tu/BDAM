...
Since DRIFT model and ITCH model take the same form (four regressors with intercept), we can write these two into one text file:
...
fit.model <- function(data, drift=TRUE) {
  ...
  subIdx <- rep(1:length(table(data$Subject)), 
                times=as.integer(table(data$Subject)))
  ...
  monitor <- c("b.mu") # Tell JAGS latent variables to monitor
  
  results <- run.jags(model="./models/model4part1.txt", monitor=monitor, data=dat.jags, 
                      n.chains=3, inits=c(inits1, inits2, inits3), burnin=200, sample=200, thin=1)
...
Assumption: the variance is homogeneous across different groups.
...
knitr::kable(summary(results), digits=6, align="c")
```
...
* $b_0$ corresponds to the global mean
* $b_i$ is interpreted as the deviation from the global mean due to the $i$th group effect
...
```{r, warning=FALSE}
dif <- (b.samples$Condition1 + b.samples$Condition4)/2 - 
  (b.samples$Condition2 + b.samples$Condition5)/2
d.density <- with(density(dif), data.frame(x, y))
ggplot(data=d.density, mapping=aes(x=x, y=y)) +
